		...
		public bool PasswordReset()
		{
			if (UsersManagementService.IsExistsUser(new Query<User>(e => e.UserName == UserName)))
			{
				...
				MailSender.SendMail(user.Email, 
                    ...
                    "Для восстановления пароля администратора пройдите по ссылке " + "<a href='" + ConfigurationManager.AppSettings["ResetPasswordUrl"] + token + "'>Восстановление пароля</a>",
                    new MailAddress(ConfigurationManager.AppSettings["PasswordSenderAddress"]));
